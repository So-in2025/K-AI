...
export const GuardianModeCard: React.FC = () => {
    const { userData, guardianState, startGuardian, stopGuardian, updateGuardianConfig, resetGuardian } = useUser();
    ...
    const [localAutoStop, setLocalAutoStop] = useState(userData?.guardianAutoStopTimer || 0);
    const [localVibration, setLocalVibration] = useState(userData?.guardianVibrationFeedback || false);

    useEffect(() => { if (analysis) setShowAnalysis(true); }, [analysis]);
    useEffect(() => { 
        setLocalTriggers((userData?.guardianTriggerWords || []).join(', '));
        setLocalAutoStop(userData?.guardianAutoStopTimer || 0);
        setLocalVibration(userData?.guardianVibrationFeedback || false);
    }, [userData, isEditingConfig]);
    ...
    const handleSaveConfig = () => {
        ...
        updateGuardianConfig({
            guardianTriggerWords: words,
            guardianAutoStopTimer: localAutoStop,
            guardianVibrationFeedback: localVibration,
        });
    ...
    const renderConfig = () => (
        <div className="mt-4 space-y-4">
            <div>
                <label className="text-sm font-medium text-slate-300">Palabras de Alerta (separadas por coma)</label>
                <input type="text" value={localTriggers} onChange={(e) => setLocalTriggers(e.target.value)} className="w-full p-2 bg-slate-700 rounded-md mt-1" placeholder="Ej: beber, consumir, recaída" />
            </div>
            <div>
                <label className="text-sm font-medium text-slate-300">Detener y Analizar por Silencio</label>
                <select value={localAutoStop} onChange={e => setLocalAutoStop(parseInt(e.target.value, 10))} className="w-full p-2 bg-slate-700 rounded-md mt-1">
                    <option value="0">Desactivado</option>
                    <option value="1">Tras 1 minuto</option>
                    <option value="2">Tras 2 minutos</option>
                    <option value="5">Tras 5 minutos</option>
                </select>
            </div>
            <div className="flex items-center">
                <input type="checkbox" id="vibrationCheck" checked={localVibration} onChange={e => setLocalVibration(e.target.checked)} className="h-4 w-4 rounded text-teal-500 bg-slate-800 border-slate-600 focus:ring-teal-500" />
                <label htmlFor="vibrationCheck" className="ml-2 text-sm text-slate-300">Vibrar al detectar alerta</label>
            </div>
            <div className="flex gap-2 mt-2">
                <button onClick={() => setIsEditingConfig(false)} className="flex-1 bg-slate-600 py-2 rounded-lg text-sm">Cancelar</button>
                <button onClick={handleSaveConfig} className="flex-1 bg-teal-600 text-white py-2 rounded-lg text-sm">Guardar Configuración</button>
            </div>
        </div>
    );
    ...
    const renderContent = () => {
        if (showAnalysis && analysis) {
            if ('isLocked' in analysis && analysis.isLocked) return <UpgradeCard />;
            return (
                <div>
                    ...
                    <button onClick={() => { setShowAnalysis(false); resetGuardian(); }} className="w-full mt-4 bg-slate-600 text-white font-semibold py-2 rounded-lg">Entendido</button>
                </div>
            );
        }
        ...
        switch (status) {
            case 'active': return (
                <div className="text-center">
                    ...
                    <div className="flex justify-center items-center h-12 space-x-2 my-2">
                        <div className="w-2 h-4 bg-teal-400 rounded-full animate-sound-bar" style={{ animationDelay: '0s' }}></div>
                        <div className="w-2 h-8 bg-teal-400 rounded-full animate-sound-bar" style={{ animationDelay: '0.2s' }}></div>
                        <div className="w-2 h-5 bg-teal-400 rounded-full animate-sound-bar" style={{ animationDelay: '0.4s' }}></div>
                    </div>
            ...
            case 'error': return (
                 <div className="text-center bg-red-900/50 border border-red-500 p-4 rounded-lg">
                    ...
                    <button onClick={resetGuardian} className="mt-3 bg-slate-600 text-white font-semibold py-1 px-4 rounded-lg text-sm">Reintentar</button>
                 </div>
            );
            default: return (
                 <>
    ...
    return (
        <div className={`bg-slate-800 p-6 rounded-2xl shadow-lg relative ${status === 'active' ? 'ring-2 ring-red-500/50' : ''}`}>
            <style>{` @keyframes sound-wave { 0%, 100% { transform: scaleY(0.4); } 50% { transform: scaleY(1); } } .animate-sound-bar { animation: sound-wave 1.2s infinite ease-in-out; } `}</style>
            ...
            <div className="flex items-center justify-between mb-3">
                ...
                {status === 'idle' && !showAnalysis && (
                    <button onClick={() => setIsEditingConfig(p => !p)} className="text-slate-400 hover:text-teal-400 p-1 rounded-full hover:bg-slate-700">
                        <SettingsIcon />
                    </button>
                )}
            </div>
